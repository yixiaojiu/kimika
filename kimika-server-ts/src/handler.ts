...
function registerReceiver(
	call: grpc.ServerUnaryCall<
		remote_pb.RegisterReceiverRequest,
		remote_pb.RegisterReceiverResponse
	>,
	...
	const res = new remote_pb.RegisterReceiverResponse()
	const id = nanoid()
...
function registerContent(
	call: grpc.ServerUnaryCall<
		remote_pb.RegisterContentRequest,
		remote_pb.RegisterContentResponse
	>,
	callback: grpc.sendUnaryData<remote_pb.RegisterContentResponse>,
) {
	const content = call.request.toObject()
	const ip = call.getPeer().split(":")[0]
	const res = new remote_pb.RegisterContentResponse()
...
async function getReceivers(
	call: grpc.ServerWritableStream<
		remote_pb.EmptyRequest,
		remote_pb.GetReceiversResponse
	>,
) {
	while (true) {
		if (!call.writable) {
		...
		const res = new remote_pb.GetReceiversResponse()
...
async function getContent(
	call: grpc.ServerWritableStream<
		remote_pb.GetContentRequest,
		remote_pb.GetContentResponse
	>,
) {
	...
	while (true) {
		...
		const contentList = Array.from(contentMap.values()).filter(
...
async function chooseReceiver(
	call: grpc.ServerWritableStream<
		remote_pb.ChooseReceiverRequest,
		remote_pb.ChooseReceiverResponse
	>,
) {
...
async function receive(
	call: grpc.ServerWritableStream<
		remote_pb.ReceiveRequest,
		remote_pb.TransferContent
	>,
) {
